...
-}
  ...
  colToColA :: EOBoard -> EOBoard
  colToColA (fnds,[],res) = (fnds,[],res)
  colToColA (fnds,cols,res)
    |elem (pCard (head hc)) (map last allTopCards) = (fnds,(topCardA ++hc): map(\n-> if head n==head topCardA then (drop (length topCardA) n) else n)tc,res)
    |otherwise =(newFnds,hc:newCols,newRes) 
    where
     hc:tc = cols
     allTopCards = map topCard tc
     topCardA =topCard (concat(filter(\n-> (pCard(head hc))==(last(topCard n))) tc))
     (newFnds,newCols,newRes) = colToColA (fnds,tc,res)

  colToResForward :: EOBoard -> EOBoard
  colToResForward (fnds,[],res) = (fnds,[],res)
  colToResForward (fnds,cols,res)
    |length hc >1 && elem (hc!!1) ((map pCard colHeads)++(map sCard (filter (not.isKing)res))) && length res <8= (fnds,(tail hc):tc,(head hc): res)
    |otherwise = (newFnds,hc:newCols,newRes)
    where
     hc:tc = cols
     colHeads = map head tc
  ...
  findMoves b@(fnds,cols,res) = filter (\n -> n/=b) allMoves --eliminate duplicates
    where
    allMoves = (colKingToCol b:resKingToCol b:resToCol b:colToCol b:colToCol b:colToColA b:colToResForward b:colToRes b:[])
